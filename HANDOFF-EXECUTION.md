...
- Tip Jar artifacts: contract+specs `eosio.contracts-tipjar` (branch
  `example/tipjar`, local-only), dapp `ultra-tipjar-dapp`.
...
- Repo: `ultra-signer-lib` (github.com/ultraio/ultra-signer-lib,
...
- Repo: `ultratest2` (github.com/ultraio/ultratest2, private;
...
`spring/build/bin/`, CDT `eosio.cdt/build`, contracts
...
- Multi-agent machine: own worktree/branch per repo; never commit to a shared checkout's
  primary branch; **never touch** `eosio.contracts` (master WIP),
