...
| Piece | Location |
| --- | --- |
| Contract + specs | `eosio.contracts-tipjar` `[internal]` — worktree of the private `eosio.contracts`, branch `example/tipjar` (local commit, never pushed) |
| Dapp | `ultra-tipjar-dapp` `[internal]` (local-only) — but §2/§4/§5 reproduce all correctness-critical code |
...
ultratest2 --contracts-dir-path=eosio.contracts-tipjar/build/contracts \
  -t eosio.contracts-tipjar/ultratests/tipjar/tipjar.spec.ts \
...
`ultra-tipjar-dapp` follows the `05` §2 structure exactly
...
setsid ultratest2 --contracts-dir-path=eosio.contracts-tipjar/build/contracts \
  -t $PWD/e2e_setup.ts --keep-alive > /tmp/tipjar-chain.log 2>&1 &

# terminal B
cd ultra-tipjar-dapp
